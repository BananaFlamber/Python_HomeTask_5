...
def player_move(candies):
    if candies > 0:
        ...
        if move <= 0:
            print("Вы попытались нас надурить, жать что вылетели из игры так и не доиграв до конца =(")
            
        elif move <= 28:
            candies = candies - move
            return bot_move(candies)    
        else:
            print("Вы попытались взять больше конфет, чем разрешено условием игры, ход переходит оппоненту!!!")
            return bot_move(candies)
    else:
